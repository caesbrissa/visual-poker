...
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ComposedChart, BarChart, Bar, PieChart, Pie, Cell } from 'recharts';
...
export default function Dashboard() {
  ...
  const sessoesFiltradas = useMemo(() =>
    filtrarPorPeriodo(sessoes, periodoGrafico),
    [sessoes, periodoGrafico]
  );

  const sessoesTabela = useMemo(() =>
    filtrarPorPeriodo(sessoes, periodoTabela),
    [sessoes, periodoTabela]
  );
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div style={{
          ...
          <p style={{ margin: '0 0 8px 0', color: '#999' }}>{data.Data || data.mes}</p>
          {data.Jogos !== undefined && (
            <p style={{ margin: '4px 0', color: '#f59e0b' }}>
              <strong>Jogos:</strong> {data.Jogos || data.jogos}
            </p>
          )}
          {data.Ganhos !== undefined && (
            <p style={{ margin: '4px 0', color: data.Ganhos >= 0 ? '#10b981' : '#ef4444' }}>
              <strong>Ganhos:</strong> {fM(data.Ganhos || data.ganhos)}
            </p>
          )}
          {data.Saldo !== undefined && (
            <p style={{ margin: '4px 0', color: '#3b82f6' }}>
              <strong>Saldo:</strong> {fM(data.Saldo)}
            </p>
          )}
        </div>
      );
    ...
    <div style={{
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {['semana', 'mes', 'ano', 'todos'].map(p => (
              <button
                ...
                style={{
                  background: periodoGrafico === p ? '#3b82f6' : 'var(--bg-button, #1a1a1a)',
                  color: periodoGrafico === p ? '#fff' : 'var(--text-secondary, #666)',
                  border: 'none',
                  padding: '8px 16px',
                  borderRadius: '8px',
                  fontSize: '12px',
                  ...
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div style={{
                        ...
                        <p style={{ margin: 0, color: data.payload.color, fontWeight: 'bold' }}>
                          {data.name}
                        </p>
                        <p style={{ margin: '5px 0 0 0', color: '#999' }}>
                          {data.value} sessões ({((data.value / sessoes.length) * 100).toFixed(1)}%)
                        </p>
                      </div>
                    );
                  }
      ...
      <div style={{
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {['semana', 'mes', 'ano', 'todos'].map(p => (
              <button
                ...
                style={{
                  background: periodoTabela === p ? '#3b82f6' : 'var(--bg-button, #1a1a1a)',
                  color: periodoTabela === p ? '#fff' : 'var(--text-secondary, #666)',
                  border: 'none',
                  padding: '6px 12px',
                  borderRadius: '6px',
                  fontSize: '11px',
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ position: 'sticky', top: 0, background: 'var(--bg-card, #0a0a0a)', zIndex: 1 }}>
              <tr style={{
                ...
                fontSize: '12px',
                textAlign: 'left',
                borderBottom: '2px solid var(--border-color, #1a1a1a)'
              }}>
            ...
            <tbody>
              {sessoesTabela.slice().reverse().map((s, i) => (
                <tr
                  key={i}
                  style={{
                    borderBottom: '1px solid var(--border-light, #0f0f0f)',
                    fontSize: '13px',
                    transition: 'background 0.2s'
                  }}
                  onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg-hover, #0f0f0f)'}
                  onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
                >
                  <td style={{ padding: '12px', color: 'var(--text-secondary, #666)' }}>{s.Data}</td>
                  <td style={{
                    padding: '12px',
                    color: s.Ganhos >= 0 ? '#10b981' : '#ef4444',
                    fontWeight: '500'
                  }}>
                    {fM(s.Ganhos)}
                  </td>
        ...
        <div style={{
          marginTop: '15px',
          padding: '10px',
          background: 'var(--bg-footer, #0f0f0f)',
          borderRadius: '8px',
          fontSize: '12px',
  ...
  return (
    <div style={{
      ...
      onMouseLeave={(e) => {
        ...
        e.currentTarget.style.borderColor = 'var(--border-color, #1a1a1a)';
      }}
    >
      <p style={{
        color: '#444',
